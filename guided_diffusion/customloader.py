...
class MRBoneDataset(torch.utils.data.Dataset):
    def __init__(self, directory, transform=None, test_flag=False):
        ...
        # ## --- Create a map of every slice --- ##
        # Iterate through the volumes to map out each individual slice
        for volume in volume_paths:
            # Load image to get its dimensions
            mr_img_nii = nibabel.load(volume['mr'])
            num_slices = mr_img_nii.shape[2] # Num axial slices
    ...
    def __len__(self):
        # ## --- Length is the total number of 2D slices --- ##
        return len(self.slice_map)

    def __getitem__(self, idx):
        # ## --- Load and process only ONE slice --- ##
